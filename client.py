...
from imagemaker import overlay_images
...
def photo_handler(m):
    ...
    save_path = "user_input_photos/background.jpg"
    with open("user_input_photos/background.jpg", "wb") as new_file:
        new_file.write(downloaded_file)
    bot.send_message(m.chat.id, f"Фото принято и обрабатывается. Пожалуйста отправьте текст для помещения на фото.")

    bot.register_next_step_handler(m, lambda m: photo_text_handler(m, path = save_path))
...
def photo_text_handler(m, path):
    ...
    if m.content_type != 'text':
        ...
        return
    background = save_path
    overlay_images(background, "foreground.png", "edited.png", m.text) 
